...
logger = get_logger("akshare-api.database")
        ...
        if self.pool is None:
            try:
                self.pool = await aiomysql.create_pool(
                    host=os.getenv("DB_HOST", "mysql-stock"),
                    port=int(os.getenv("DB_PORT", 3306)),
                    user=os.getenv("DB_USER", "root"),
                    password=os.getenv("DB_PASSWORD", "stock123"),
                ...
                logger.info("MySQL 异步连接池初始化完成 (akshare-api)")
