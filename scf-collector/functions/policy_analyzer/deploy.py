...
# 2. 目标函数名称
func_name = 'stock-policy-analyzer'
...
def package_code():
    ...
    zip_path = os.path.join(zip_dir, 'policy_analyzer_code.zip')
...
def setup_triggers():
    """配置每 5 分钟触发一次的高频 AI 提取分析 Timer 触发器"""
    cred = credential.Credential(secret_id, secret_key)
    client = scf_client.ScfClient(cred, region)
    
    t = {
        "name": "PolicyAnalyzer5MinTimer",
        "cron": "0 */5 * * * * *",
        "payload": {"op": "analyze_policies"}
    }
