...
# 2. 目标函数名称
func_name = 'stock-policy-notifier'
...
def package_code():
    ...
    zip_path = os.path.join(zip_dir, 'policy_notifier_code.zip')
...
def setup_triggers():
    """配置每 15 分钟触发一次的高频政策分发预警 Timer 触发器"""
    cred = credential.Credential(secret_id, secret_key)
    client = scf_client.ScfClient(cred, region)
    
    t = {
        "name": "PolicyNotifier15MinTimer",
        "cron": "0 */15 * * * * *",
        "payload": {"op": "notify_policies"}
    }
