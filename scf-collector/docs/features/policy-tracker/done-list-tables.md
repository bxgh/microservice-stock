...
| 表名 | 业务说明 | 核心变更与 AC 达成情况 | 状态 |
| :--- | :--- | :--- | :--- |
| `dwd_policy_analysis` | 政策精细化分析与相似度对照表 | 1. 规则直切落库支持 SimHash 达成<br>2. 初筛阻断路径落库 SimHash 达成<br>3. 64位纯Python位运算汉明距离检索达成 | **已就绪 (Completed)** |

### 4.1 核心相似度指纹字段映射

对于 `dwd_policy_analysis` 表，已实现字段与业务实体对照关系：

| 字段名 | 类型 | 约束 / 索引 | 业务映射说明 |
| :--- | :--- | :--- | :--- |
| `core_segment_simhash` | `CHAR(16)` | `DEFAULT NULL` | 核心段落的 64 位 SimHash 十六进制指纹 |
