...
> **Status**: Designing  
> **Source of Truth**: [E400_SCF_Collector_Implementation.md](file:///home/ubuntu/microservice-stock/scf-collector/docs/E400_SCF_Collector_Implementation.md)

## 1. 背景与目标

### 1.1 背景
当前 `scf-collector` 已完成元数据（交易日历、股票列表）的同步逻辑，并具备基础的 VPC 穿透与云端部署能力。然而，核心的行情数据（K 线）、财务基本面、资金流及公告事件等 30+ 张表尚未实现生产级的自动化采集。
根据 v0.2 补遗，`stock_kline_daily` 确认不复权，需紧急补齐复权因子。
...
### E400-S1: P0 核心行情同步 (Daily K-Lines & Factors)
**描述**：正式实现个股及指数的日 K 线同步（不复权），并补齐复权因子与行业成员维表。
...
**描述**：采集每日 PE/PB 估值、停复牌、基金净值及异动原因。
...
## 3. 交付物清单
1. **Epic 文档**: `docs/design/SCF/E400_SCF_Collector_Implementation.md` (本文件)
