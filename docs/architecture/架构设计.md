...
graph TB
    ...
    subgraph Cloud["云端服务器 124.221.80.250"]
        StockCodes[stock-codes-api<br/>Port: 8000]
        BaoStock[baostock-api<br/>Port: 8001]
        PyWencai[pywencai-api<br/>Port: 8002]
        AkShare[akshare-api<br/>Port: 8003]
        MySQL[(MySQL 5.7<br/>Tencent Cloud)]
    end
    
    subgraph DataSource["外部数据源"]
        AkShareData[东方财富/新浪等]
        BaoStockData[baostock.com:10030]
        WencaiData[同花顺问财]
        LocalDB[a_share_codes.db]
    end
    
    Client -->|通过代理| Proxy
    Proxy -->|HTTP| StockCodes
    Proxy -->|HTTP| BaoStock
    Proxy -->|HTTP| PyWencai
    Proxy -->|HTTP| AkShare
    
    StockCodes -->|SQLite| LocalDB
    AkShare -->|爬取| AkShareData
    BaoStock -->|TCP连接| BaoStockData
    BaoStock -->|异步写入/断点续传| MySQL
...
| `/api/v1/sync/status` | GET | 查看同步进度 | - |
| `/api/v1/sync/reset` | POST | 重置同步进度 | - |

**数据持久化策略**:
- **存储**: 腾讯云 MySQL 5.7
- **机制**: 异步 `BackgroundTasks` + `aiomysql` 连接池
- **特性**: 
  - **断点续传**: 使用 `sync_progress` 表记录处理索引，支持崩溃后自动恢复
  - **增量同步**: 自动识别库内最新日期，仅抓取缺失数据
...
| 组件 | 版本/选型 | 说明 |
|-----|---------|------|
| 基础镜像 | python:3.12-slim | 轻量级且包含必要编译工具 |
| Web框架 | FastAPI | **必须使用** |
| 异步数据库 | aiomysql | 用于 MySQL 异步操作 |
| 数据库 | MySQL 5.7 | 腾讯云 CDB |
