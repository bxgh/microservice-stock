...
- **交付物闭环 (废除 REPORT.html)**: 
  ...
  - **同时，必须在 `implementation_logs/E{N}/` 目录下增量更新或新建 `state_E{N}.json` 状态存证文件**，以机器可读（AI-Native）的结构化形式记录当前物理产出与下游交接备注。
...
9. **微信小程序网关 (wxch-gateway) 专属交付与文档原则 (wxch-gateway Delivery & Documentation Specification)**:
   - **API 契约一致性门禁**: 网关微服务涉及的所有外部 API 变更，在 Story 交付时**必须**在其对应的交付日志目录下产出 high-fidelity 的 HTTP REST API 契约文档 `API.md`（如 `implementation_logs/E{N}/S{M}/API.md`），明确定义：Base URL、安全 JWT 鉴权规则、入参校验模型、成功响应体及防重复提交拦截等异常响应。
   - **真源 HTML 编译与门户同步**: `API.md` 绝对禁止手写或由 AI 重复生成 HTML 副本，必须通过 `python scripts/md_to_html_premium.py` 进行标准编译产出 `API.html`，且自动通过索引脚本同步更新挂接到全局文档 Portal（`docs/index.html`）和 `wxch-gateway` 的局部门户中。
   - **数据库变更追溯同步**: 针对新表上线或表结构字段扩展，必须同步在 `docs/features/wechat-gateway/design/` 的设计逻辑文档中完整补充，并与全局 `docs/design/股市日记/` 下的 `表结构.md` 和 `表初始化.md`（包含 DDL 及清理重置 SQL）进行 100% 对齐。
   - **安全鉴权防护规范**: 所有新增的打卡、解读、个人中心等网关核心接口，在控制器（`api/*.py`）中必须强制声明激活原有 JWT 拦截依赖守卫（`Depends(get_current_user_id)`），确保无鉴权漏洞。
...
- ❌ 使用 `re.sub` 进行模板/JSON替换时直接传递含转义字符的字符串 (应使用 `lambda` 包装以避免反斜杠被二次解析)
- ❌ **读取或分析 `.html` 格式的文档** (AI Agent 的唯一真源必须是 `.md` 或 `.json`。`.html` 页面专为人类浏览器排版服务，AI 严禁对其发起 `view_file` 或 `grep_search` 等工具读取，以节省高达 90% 的 Context Token 消耗)
...
| 2026-05-19 | v2.2 | **网关交付规范**: 在 5.7 节增补网关微服务 wxch-gateway 专属交付与文档原则，强化 API 契约、HTML 编译、数据库同步与安全鉴权规范。 |
| 2026-05-19 | v2.3 | **Token 优化规范**: 将所有自动生成的 `*.html` 文件列入 `.gitignore` 过滤列表，并在第 6 节注入“禁止 AI 读取 HTML 文件”的硬约束，降低 90% 的 Context Token 损耗。 |
