...
async def test_user_profile_and_diary():
    ...
    async with AsyncClient(app=app, base_url="http://test") as ac:
        ...
        # 3. Test Diary List
        list_res = await ac.get("/api/v1/diaries", headers=headers)
        ...
        # 5. Get Diary
        get_res = await ac.get(f"/api/v1/diaries/{new_id}", headers=headers)
        assert get_res.status_code == 200
        
        # 6. Delete Diary
        del_res = await ac.delete(f"/api/v1/diaries/{new_id}", headers=headers)
