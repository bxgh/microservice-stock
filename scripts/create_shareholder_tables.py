...
sys.path.append(os.path.join(project_root, 'stock-manager-api'))

try:
    from app.config import settings
except ImportError as e:
    print(f"Error importing config: {e}")
    # Fallback/Mock for testing if env not fully set
    class MockSettings:
    ...
    async with pool.acquire() as conn:
        async with conn.cursor() as cur:
            ...
            CREATE TABLE IF NOT EXISTS `stock_shareholder_count` (
                ...
                `holder_change_pct` decimal(24,6) DEFAULT NULL COMMENT '户数变动比例',
