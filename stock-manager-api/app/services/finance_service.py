...
class FinanceService:
    ...
    async def get_financial_reports(
            self, ts_code: str, limit: int = 40) -> Dict[str, Any]:
        """从数据库查询三大报表数据"""
        ...
        try:
            ...
            if not data or "indicators" not in data:
                return {"ts_code": ts_code, "success": False,
                        "message": "No indicator data from source"}
    ...
    async def _save_financial_indicators(
            ...
            rows.append(
                (ts_code,
                 item.get("report_date"),
                    ...
                    item.get("total_equity_ato_parent"),
                    item.get("monetary_funds"),
                    item.get("accounts_receivable"),
                    item.get("notes_receivable"),
                    item.get("inventory"),
                    item.get("goodwill"),
                    item.get("short_term_borrowings"),
                    item.get("long_term_borrowings"),
                    item.get("total_non_current_assets"),
                    item.get("total_current_assets"),
                    item.get("total_non_current_liabilities"),
            ...
            rows.append(
                (ts_code,
                 item.get("report_date"),
                    ...
                    item.get("total_operating_cost"),
                    item.get("operating_cost"),
                    item.get("selling_expenses"),
                    item.get("administrative_expenses"),
                    item.get("financial_expenses"),
                    item.get("research_expenses"),
                    item.get("operating_profit"),
                    item.get("total_profit"),
                    item.get("net_profit"),
                    item.get("parent_net_profit"),
                    item.get("deducted_net_profit"),
                    item.get("ebit"),
                    item.get("ebitda")))
        await db.execute_many(sql, rows)
        return len(rows)

    async def _save_cash_flow_statements(
            self, ts_code: str, data: List[Dict[str, Any]]) -> int:
        if not data:
            return 0
        ts_code = normalize_ts_code(ts_code)
        ...
        for item in data:
            rows.append(
                (ts_code,
                 item.get("report_date"),
                    item.get("notice_date"),
                    item.get("net_operating_cash_flow"),
                    item.get("net_investing_cash_flow"),
                    item.get("net_financing_cash_flow"),
                    item.get("capex"),
                    item.get("free_cash_flow"),
                    item.get("cash_and_equivalents_at_end")))
        await db.execute_many(sql, rows)
        return len(rows)
