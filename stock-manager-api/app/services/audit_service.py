...
class AuditService:
    ...
    async def get_audit_weekly(self, week: str = "current") -> Dict[str, Any]:
        ...
        try:
            if week == "current":
                ...
                try:
                    ...
                    start_date = datetime.datetime.strptime(
                        f"{year_part}-W{week_part}-1", "%G-W%V-%u").date()
                    week_label = week
            ...
            mysql_counts = {
                row[0].strftime("%Y-%m-%d") if isinstance(
                    row[0], (datetime.date, datetime.datetime)) else str(
                    row[0]): row[1] for row in mysql_res}
            ...
            for cal_row in cal_rows:
                d_date = cal_row[0].strftime("%Y-%m-%d") if isinstance(
                    cal_row[0], (datetime.date, datetime.datetime)) else str(cal_row[0])
    ...
    async def get_gate_audits(
            self, trade_date: str = None, limit: int = 50) -> Dict[str, Any]:
        """获取数据闸门审计记录"""
        try:
            params = []
