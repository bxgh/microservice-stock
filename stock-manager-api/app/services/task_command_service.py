...
class TaskCommandService:
    ...
    async def create_command(self, task_id: str, params: Dict[str, Any]) -> int:
        """
        创建任务指令
        :param task_id: 任务标识符
        :param params: 参数字典
    ...
    async def get_commands(self, task_id: Optional[str] = None, status: Optional[str] = None, limit: int = 20) -> List[Dict[str, Any]]:
        """
        查询指令列表
        """
        try:
    ...
    async def get_command_by_id(self, command_id: int) -> Optional[Dict[str, Any]]:
        """
        查询单个指令详情
        """
        try:
