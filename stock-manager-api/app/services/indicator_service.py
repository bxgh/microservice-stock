...
class IndicatorService:
    ...
    async def calculate_l2_indicators_full(self, target_date: str = None):
        ...
        try:
            ...
            date_5d = past_dates[4] if len(past_dates) >= 5 else (past_dates[-1] if past_dates else None)
            date_20d = past_dates[19] if len(past_dates) >= 20 else (past_dates[-1] if past_dates else None)
            ...
            if dates:
                date_str_list = [d.strftime("%Y-%m-%d") if hasattr(d, 'strftime') else str(d) for d in dates]
                ...
                for f_code, l_idx, s_idx, f_name in factors:
                    ...
                    if td_str in l_dict and td_str in s_dict:
                        ...
                        s_5d = sum(l_dict.get(d) - s_dict.get(d) for d in date_str_list[:5] if d in l_dict and d in s_dict)
                        s_20d = sum(l_dict.get(d) - s_dict.get(d) for d in date_str_list if d in l_dict and d in s_dict)
