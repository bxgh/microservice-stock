...
async def get_financial_reports(
    ...
    try:
        ...
        if not data["balance_sheets"] and not data["income_statements"]:
            raise HTTPException(status_code=404, detail=f"未找到代码 {code} 的财务报表数据")
        return data
...
@router.get("/indicators/{code}", response_model=FinancialIndicatorsListResponse)
async def get_financial_indicators(
    ...
    try:
        ...
        if not data["indicators"]:
            raise HTTPException(status_code=404, detail=f"未找到代码 {code} 的财务衍生指标数据")
        return data
    except HTTPException:
        raise
    except Exception as e:
...
@router.post("/sync-indicators/{code}", response_model=SyncFinanceIndicatorsResult)
