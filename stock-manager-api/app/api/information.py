...
@router.get("/analyst-ranks/{code}", response_model=List[AnalystRankResponse])
async def get_analyst_ranks(code: str, limit: int = 50):
    """查询机构评级历史"""
    service = InformationService()
    return await service.get_analyst_ranks(code, limit)
...
@router.get("/forecasts/{code}", response_model=List[PerformanceForecastResponse])
async def get_forecasts(code: str, limit: int = 20):
    """查询业绩预告历史"""
    service = InformationService()
    return await service.get_forecasts(code, limit)
...
@router.get("/sentiment/{code}", response_model=List[SentimentDailyResponse])
async def get_sentiment(code: str, limit: int = 30):
    """查询市场热度历史"""
    service = InformationService()
    return await service.get_sentiment(code, limit)

@router.post("/sentiment/sync-fetch/{code}", response_model=SyncResult)
async def sync_sentiment_from_src(code: str):
    """从数据源抓取并同步个股热度"""
    service = InformationService()
    count = await service.sync_sentiment_from_akshare(code=code)
